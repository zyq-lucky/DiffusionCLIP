    ...
    def edit_images_from_dataset(self):
        ...
        for mode in ['test']:
            ...
            for step, data in enumerate(loader):
                ...
                # tvu.save_image((x0 + 1) * 0.5, os.path.join(self.args.image_folder, f'{mode}_{step}_0_orig.png'))
                # tvu.save_image(mask, os.path.join(self.args.image_folder, f'{mode}_{step}_0_orig_mask.png'))
                x = x0.clone()
                with torch.no_grad():
                    ...
                    img_lat_pairs.append([x0, x.detach().clone(),x_lat.detach().clone(),mask,data['name']])
        ...
        for mode in eval_modes:
            ...
            img_lat_pairs = img_lat_pairs_dic[mode]
            for step, (x0, x_id,x_lat, mask,paths) in enumerate(img_lat_pairs):
                x0_0 = x0.clone()
                with torch.no_grad():
                    # x = x_lat.clone()
                    a = (1 - self.betas).cumprod(dim=0)
                    for it in range(self.args.n_iter):
                        ...
                        e = torch.randn_like(x0)
                        for bs_i in range(x.shape[0]):
                            tvu.save_image((e[bs_i:bs_i + 1] + 1) * 0.5,
                                os.path.join(self.args.image_folder.replace('image_samples','mid'),
                                                f'{paths[bs_i][:-4]}_{it}_noise.png'),nrow=1)
                        x = x0 * a[self.args.t_0 - 1].sqrt() + e * (1.0 - a[self.args.t_0 - 1]).sqrt()
                        with tqdm(total=len(seq_test), desc=f"Eval iteration") as progress_bar:
                            for i, j in zip(reversed(seq_test), reversed(seq_test_next)):
                                t = (torch.ones(n) * i).to(self.device)
                                t_next = (torch.ones(n) * j).to(self.device)
                                # for bs_i in range(x.shape[0]):
                                #     tvu.save_image((x[bs_i:bs_i + 1] + 1) * 0.5,
                                #         os.path.join(self.args.image_folder.replace('image_samples','mid'),
                                #                         f'{paths[bs_i][:-4]}_{it}_{int(t[0])}.png'),nrow=1)
                                x = denoising_step(x, t=t, t_next=t_next, models=models,
                                                    logvars=self.logvar,
                                                    sampling_type=self.args.sample_type,
                                                    b=self.betas,
                                ...
                                if j > 150:  
                                    x_ = x0 * a[j].sqrt() + e * (1.0 - a[j]).sqrt()
                                    x = mask * x_ + (1 - mask) * x
                                progress_bar.update(1)
                                
                        
                        print(f"Eval {step}",time.time() - start)
                        tvu.save_image((torch.cat([x0_0,x0,mask.repeat(1,3,1,1),x],dim = 3) + 1) * 0.5,
                                    os.path.join(self.args.image_folder,
                                                    f'{mode}_{step}_2_clip_ngen{self.args.n_test_step}_mrat{self.args.model_ratio}_{it}.png'),nrow=1)
                        x0 = x.clone()
                        for bs_i in range(x.shape[0]):
                            tvu.save_image((x[bs_i:bs_i + 1] + 1) * 0.5,
                                os.path.join(self.args.image_folder.replace('image_samples','single'),paths[bs_i].replace(".png",f"_{it}.png")))

        

    def edit_one_image(self,models = []):
        # ----------- Data -----------#
        n = self.args.bs_test
        ...
        with torch.no_grad():
            x_lat_path = self.args.img_path.replace('.jpg','.pth').replace('.png','.pth').replace('CelebA-HQ-img','CelebA-HQ-noised-img')
            ...
            #---------------- Invert Image to Latent in case of Deterministic Inversion process -------------------#
                if self.args.deterministic_inv:
                    ...
                    e = torch.randn_like(x0)
                    a = (1 - self.betas).cumprod(dim=0)
                    x_lat = x0 * a[self.args.t_0 - 1].sqrt() + e * (1.0 - a[self.args.t_0 - 1]).sqrt()

            else:
                print('Latent exists.')
                x_lat = torch.load(x_lat_path).to(x0.device)
            ...
            for it in range(self.args.n_iter):
                ...
                #     # e = torch.randn_like(x0)
                #     # a = (1 - self.betas).cumprod(dim=0)
                #     # x = x_lat * a[self.args.t_0 - 1].sqrt() + e * (1.0 - a[self.args.t_0 - 1]).sqrt()
                #     # w = 0.5
                #     # x = w * x_lat + (1 - w) * x
                ...
                #     x = x0 * a[self.args.t_0 - 1].sqrt() + e * (1.0 - a[self.args.t_0 - 1]).sqrt()
                    ...
                    #     a = (1 - self.betas).cumprod(dim=0)
                    #     x = x0 * a[self.args.t_0 - 1].sqrt() + e * (1.0 - a[self.args.t_0 - 1]).sqrt()
                    #     torch.save(x.cpu(), x_lat_path)
                    # else:
                    #     x = torch.load(x_lat_path).to(x0.device)
